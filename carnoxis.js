...
const carnoxisconfig = require('./carnoxisconfig.json');
...
var prefix = carnoxisconfig.prefix;
...
client.elevation = message => {
  if(!message.guild) {
	return; }
  let permlvl = -carnoxisconfig.varsayilanperm  ;
  ...
  if(message.author.id === carnoxisconfig.sahip) permlvl = 7;
  return permlvl;
};
...
client.login(carnoxisconfig.token)
...
.setTitle('Carnoxis - Autorol')

.setColor("GREEN")

.setDescription(`Joining the Server **${member}** Named User Successfully Role Given \`${rol2.name}\`.`)
...
client.on("message", async message => {
  ...
  if (db.has(`sayac_${message.guild.id}`) === true) {
    if (db.fetch(`sayac_${message.guild.id}`) <= message.guild.members.cache.size) {
      const embed = new Discord.MessageEmbed()
        .setTitle(`Congratulations ${message.guild.name}!`)
        .setDescription(`Başarıyla \`${db.fetch(`sayac_${message.guild.id}`)}\` we reached the user! The counter has been reset!`)
        .setColor("RANDOM");
      message.channel.send(embed);
      message.guild.owner.send(embed);
      db.delete(`sayac_${message.guild.id}`);
...
client.on("guildMemberRemove", async member => {
  ...
  if (db.has(`sKanal_${member.guild.id}`) == false) return;
    ...
    member.guild.channels.cache.get(channel).send(`**${member.user.tag}** Left the server! \`${db.fetch(`sayac_${member.guild.id}`)}\` stop being a member \`${db.fetch(`sayac_${member.guild.id}`) - member.guild.memberCount}\` member remained!`);
});
client.on("guildMemberAdd", async member => {
  const channel = db.fetch(`sKanal_${member.guild.id}`);
  if (db.has(`sayac_${member.guild.id}`) == false) return;
  if (db.has(`sKanal_${member.guild.id}`) == false) return;
    ...
    member.guild.channels.cache.get(channel).send(`**${member.user.tag}** Joined the Server :tada:! \`${db.fetch(`sayac_${member.guild.id}`)}\` stop being a member \`${db.fetch(`sayac_${member.guild.id}`) - member.guild.memberCount}\` member remained!`);
});
